...
function ManageBranches() {
    ...
    const sortedBranches = [...branches].sort((a, b) => b.id - a.id);
    const indexOfLastBranch = currentPage * branchesPerPage;
    const indexOfFirstBranch = indexOfLastBranch - branchesPerPage;
    const currentBranches = sortedBranches.slice(indexOfFirstBranch, indexOfLastBranch);
    ...
    return (
        <div className="space-y-6">
            <div className="flex sm:items-center justify-between flex-col sm:flex-row gap-4">
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('seller.br.title')}</h1>
                <button
                ...
                >
                    <Plus className="w-4 h-4" /> {t('seller.br.add')}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    ...
                    return (
                        <div key={branch.id} className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm p-6 flex flex-col justify-between gap-4">
                                ...
                                <div className="space-y-2 text-sm text-slate-500 dark:text-slate-400">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {managerName ? (
                                            ...
                                            <span className="italic text-slate-400">{t('seller.br.no_mgr_assigned')}</span>
                                        )}
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-slate-800">
                                ...
                                >
                                    <Edit2 className="w-3.5 h-3.5" /> {t('seller.br.edit')}
                                </button>
                                <button
                                ...
                                >
                                    <Trash2 className="w-3.5 h-3.5" /> {t('seller.br.delete')}
                                </button>
                            </div>
                        </div>
                    );
                ...
                {branches.length === 0 && (
                    <div className="col-span-full text-center py-10 text-slate-400">{t('seller.br.empty')}</div>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl overflow-hidden">
                        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-800">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">{editingBranch ? t('seller.br.edit') : t('seller.br.add')}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t('seller.br.name')}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t('seller.br.location')}</label>
                                <input type="text" name="location" required value={formData.location} onChange={handleChange}
                                    className="block w-full rounded-xl border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-primary-500 focus:ring-primary-500 sm:text-sm py-3 px-4"
                                    placeholder="Full address" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t('seller.br.contact')}</label>
                                <input type="tel" name="contact" required value={formData.contact} onChange={handleChange}
                                    className="block w-full rounded-xl border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-primary-500 focus:ring-primary-500 sm:text-sm py-3 px-4"
                                    placeholder="017..." />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t('seller.br.manager')}</label>
                                ...
                                >
                                    <option value="">{t('seller.br.no_manager')}</option>
                                    ...
                                    {editingBranch?.managerId && !availableManagers.find(m => m.id === editingBranch.managerId) && (() => {
                                        ...
                                        return currentManager ? <option key={currentManager.id} value={currentManager.id}>{currentManager.name} ({t('seller.br.current')})</option> : null;
                                    })()}
                                </select>
                            </div>
                            <button type="submit" className="w-full py-3 px-4 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-medium transition-colors shadow-sm">
                                {editingBranch ? t('seller.br.save') : t('seller.br.create')}
                            </button>
                        </form>
                    </div>
                </div>
